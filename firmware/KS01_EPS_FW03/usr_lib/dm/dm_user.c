...
struct dm_var_s dm_var;
 ...
 __no_init struct dm_cfg_s dm_cfg@user_set_addr;
...
int dm_var_write_callback(uint16_t offset, int size) {
        ...
        if(dm_var.deploy)
	{
        ...
        if(dm_var.heater != (uint8_t)(dm_cfg.heat_valid&0x00ff))
	{
          chang_cfg(CFG_OFF_HEATERVALID, (uint16_t)dm_var.heater);
	}
	
	if(dm_var.deploy != (uint8_t)(dm_cfg.obc_run_flag&0x0001))
	{
#ifdef OBC_PRESERVE
          chang_cfg(CFG_OFF_OBCRUNFLAG, (uint16_t)dm_var.deploy);
#endif
	}
...
int dm_cfg_write_callback(uint16_t offset, int size) {
	...
	//chang_cfg((uint32_t)(&dm_cfg + offset),*(uint16_t*)(&dm_cfg + size));
