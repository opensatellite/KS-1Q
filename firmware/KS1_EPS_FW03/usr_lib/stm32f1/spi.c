...
void __spi_init_pins(SPI_TypeDef *spi)
{
    if(spi == SPI1)
    {
        ...
        pinhigh(PA4);
        #endif
        pinmodespd(PA5,AFPUSHPULL,pin_10MHz);
        pinmodespd(PA6,AFPULLUP,pin_10MHz);
        pinmodespd(PA7,AFPUSHPULL,pin_10MHz);
    }
    
    if(spi == SPI2)
    {
        #ifdef SPI_INIT_CS    
        pinmodespd(PB12,PUSHPULL,pin_2MHz);
        pinhigh(PB12);
        #endif
        pinmodespd(PB13,AFPUSHPULL,pin_10MHz);
        pinmodespd(PB14,AFPULLUP,pin_10MHz);
        pinmodespd(PB15,AFPUSHPULL,pin_10MHz);
    }
}
