...
inline uint16_t CONST csp_hton16(uint16_t h16) {
...
inline uint16_t CONST csp_ntoh16(uint16_t n16) {
	return csp_hton16(n16);
}

/* Convert 32-bit number from host byte order to network byte order */
inline uint32_t CONST csp_hton32(uint32_t h32) {
#ifdef CSP_BIG_ENDIAN
	return h32;
#else
...
inline uint32_t CONST csp_ntoh32(uint32_t n32) {
	return csp_hton32(n32);
}

/* Convert 64-bit number from host byte order to network byte order */
inline uint64_t CONST csp_hton64(uint64_t h64) {
#ifdef CSP_BIG_ENDIAN
	return h64;
#else
...
/* Convert float from host byte order to network byte order */
inline float CONST csp_ntohflt(float f) {
	return csp_htonflt(f);
}

/* Convert double from host byte order to network byte order */
inline double CONST csp_htondbl(double d) {
#ifdef CSP_BIG_ENDIAN
	return d;
#else
	union v {
...
/* Convert float from host byte order to network byte order */
inline double CONST csp_ntohdbl(double d) {
