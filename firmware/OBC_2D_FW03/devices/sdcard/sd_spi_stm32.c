...
static void interface_speed( enum speed_setting speed )
{
	...
	if ( speed == INTERFACE_SLOW ) {
		...
		tmp = ( tmp & ~SPI_BaudRatePrescaler_256 ) | __spi_divider(SPI_SD_SPEED);
	}
	SPI_SD->CR1 = tmp;
}
...
#if (SD_HAS_WP == 1)
static inline void socket_wp_init(void)
{
	pinmode(PIN_SD_WP,PULLUP);
}
static inline DWORD socket_is_write_protected(void)
{
	return ( pinread(PIN_SD_WP) ? 1 : 0);
}
#else
static inline void socket_wp_init(void) {;}
static inline DWORD socket_is_write_protected(void) {return 0;}
#endif

/* Socket's Card-Detect Pin: high = socket empty, low = card inserted */
#if (SD_HAS_CD == 1)
static void socket_cd_init(void)
{
	pinmode(PIN_SD_CD,PULLUP);
}
static inline DWORD socket_is_empty(void)
{
	return (pinread(PIN_SD_CD) ? 1 : 0);
}
#else
static void socket_cd_init(void) {;}
static inline DWORD socket_is_empty(void) {return 0;}
#endif


#if (SD_PWR_SWITCHABLE == 1)
static void card_power(bool on)		/* switch FET for card-socket VCC */
{
	pinmode(PIN_SD_PWR,OUTPUT);
	if(on) {pinlow(PIN_SD_PWR);}
	else {pinhigh(PIN_SD_PWR);pinlow(PIN_SD_CS);}
}
#else
static void card_power(bool on) {on=on;}
#endif
...
static int chk_power(void)		/* Socket power state: 0=off, 1=on */
{
	...
	#else
static int chk_power(void) {return 1;}
	#endif
#endif
...
#define xmit_spi(dat)  spi_rw(SPI_SD, dat)
...
BYTE rcvr_spi (void)
{
	return spi_rw(SPI_SD, 0xff);
}

/* Alternative macro to receive data fast */
#define rcvr_spi_m(dst)  *(dst)=spi_rw(SPI_SD, 0xff)
...
void power_on (void)
{
	...
	if(!disk_timer_mounted)
	{
		disk_timer_mounted = 1;
		//systick_attach(disk_timerproc_helper);
	}
}
...
void power_off (void)
{
	...
	//systick_detach(disk_timerproc_helper);
	disk_timer_mounted = 0;
